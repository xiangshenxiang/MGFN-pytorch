...
from openvqa.utils1.ans_punct import prep_ans
...
class DataSet(BaseDataSet):
    def __init__(self, __C):
        ...
        #/home/cqh/xsx/VQA/openvqa/data/vqa/feats
        frcn_feat_path_list = ['/home/cqh/xsx/VQAVQA/openvqa/data/vqa/feats/train2014/COCO_train2014_000000474858.jpg.npz']
        ...
        stat_ques_list = [{'image_id': 474858, 'question': 'What color is the board?', 'question_id': 474858026}]
        ...
        self.ques_list = []
        self.ans_list = []
        self.ques_list = [{'image_id': 474858, 'question': 'What color is the board?', 'question_id': 474858026}]

        self.ans_list = [{'answer_type': 'other', 'multiple_choice_answer': 'white',
                          'answers': [{'answer': 'white', 'answer_confidence': 'yes', 'answer_id': 1},
                                      {'answer': 'red and white', 'answer_confidence': 'yes', 'answer_id': 2},
                                      {'answer': 'white and red', 'answer_confidence': 'yes', 'answer_id': 3},
                                      {'answer': 'white,red', 'answer_confidence': 'yes', 'answer_id': 4},
                                      {'answer': 'white', 'answer_confidence': 'yes', 'answer_id': 5},
                                      {'answer': 'white', 'answer_confidence': 'yes', 'answer_id': 6},
                                      {'answer': 'white and red', 'answer_confidence': 'yes', 'answer_id': 7},
                                      {'answer': 'white', 'answer_confidence': 'yes', 'answer_id': 8},
                                      {'answer': 'white', 'answer_confidence': 'yes', 'answer_id': 9},
                                      {'answer': 'white', 'answer_confidence': 'maybe', 'answer_id': 10}],
                          'image_id': 474858, 'question_type': 'what color is the', 'question_id': 474858026}]
        ...
        # Tokenize
        self.token_to_ix, self.pretrained_emb = self.tokenize(stat_ques_list1, __C.USE_GLOVE)
    ...
    def tokenize(self, stat_ques_list, use_glove):
        ...
        if use_glove:
            ...
            #原来的glove
            spacy_tool = en_vectors_web_lg.load()#<spacy.lang.en.English object at 0x7f518af098d0>
            pretrained_emb.append(spacy_tool('PAD').vector)
            pretrained_emb.append(spacy_tool('UNK').vector)
            pretrained_emb.append(spacy_tool('CLS').vector)

            #2022/4/26改成bert
            # encoded_input = tokenizer('PAD', return_tensors='pt')
            # pretrained_emb.append(model(**encoded_input).pooler_output.detach().squeeze(0).numpy())
            # encoded_input = tokenizer('UNK', return_tensors='pt')
            # pretrained_emb.append(model(**encoded_input).pooler_output.detach().squeeze(0).numpy())
            # encoded_input = tokenizer('CLS', return_tensors='pt')
            # pretrained_emb.append(model(**encoded_input).pooler_output.detach().squeeze(0).numpy())
        ...
        for ques in stat_ques_list:
            ...
            for word in words:
                if word not in token_to_ix:
                    ...
                    if use_glove:#glove 换成了 bert
                        # encoded_input = tokenizer(word, return_tensors='pt')
                        # pretrained_emb.append(model(**encoded_input).pooler_output.detach().squeeze(0).numpy())

                        pretrained_emb.append(spacy_tool(word).vector)
        pretrained_emb = np.array(pretrained_emb)
        # print("1111",token_to_ix)
        # print("2222",pretrained_emb.shape)
