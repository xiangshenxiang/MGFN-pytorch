...
class MHAtt(nn.Module):
    ...
    def att(self, value, key, query, mask):
        ...
        att_map = F.softmax(scores, dim=-1)
        att_map = self.dropout(att_map)
        # print(att_map.shape)#64 8 100 100 100 14 14 14
...
class SAoA(nn.Module):
    def __init__(self, __C, size=1024):
        ...
        self.gate1 = FC(__C.HIDDEN_SIZE, __C.HIDDEN_SIZE, dropout_r=__C.DROPOUT_R, use_relu=True)
        self.gate2 = FC(__C.HIDDEN_SIZE, __C.HIDDEN_SIZE, dropout_r=__C.DROPOUT_R, use_relu=True)
        self.gate3 = FC(__C.HIDDEN_SIZE, __C.HIDDEN_SIZE, dropout_r=__C.DROPOUT_R, use_relu=True)
        self.gate4 = FC(__C.HIDDEN_SIZE, __C.HIDDEN_SIZE, dropout_r=__C.DROPOUT_R, use_relu=True)

    def forward(self, y, arg1, y_mask, arg2, arg3):
        y1 = self.mhatt(v=y, k=y, q=y, mask=y_mask)
        I = self.gate1(y1)+self.gate3(y)

        G = F.sigmoid(self.gate2(y1)+self.gate4(y))
        resy = torch.mul(I, G)

        resy = self.norm(y + self.dropout(
            resy
        ))
...
class SA(nn.Module):
    ...
    def forward(self, y, arg1, y_mask, arg2, arg3):#[32, 14, 512])
...
class RSAoA(nn.Module):
    def __init__(self, __C, size=1024):
        super(RSAoA, self).__init__()
        ...
        self.gate3 = FC(__C.HIDDEN_SIZE, __C.HIDDEN_SIZE, dropout_r=__C.DROPOUT_R, use_relu=True)
        self.gate4 = FC(__C.HIDDEN_SIZE, __C.HIDDEN_SIZE, dropout_r=__C.DROPOUT_R, use_relu=True)
    def forward(self, y, arg1, y_mask, arg2, rela):
        y1 = self.mhatt(y,y,y,y_mask,rela)
        I = self.gate1(y1) + self.gate3(y)
        G = F.sigmoid(self.gate2(y1) + self.gate4(y))
        resy = torch.mul(I, G)
        resy = self.norm(y + self.dropout(
...
class GAoA(nn.Module):
    ...
    def forward(self, x, y, x_mask, y_mask, rela):
        x1 = self.mhatt(v=y, k=y, q=x, mask=y_mask)
        I = self.gate1(x1) + self.gate3(x)

        G = F.sigmoid(self.gate2(x) + self.gate4(x1))
        resx = torch.mul(I, G)
        resx = self.norm(x + self.dropout(
            resx
        ))
...
# ------------------------------------------------
# --- Encoder-Decoder Architecture of MGFN ---
# ------------------------------------------------

class MGFN_ED(nn.Module):
    def __init__(self, __C):
        super(MGFN_ED, self).__init__()
        enc = __C.ARCH['enc']
        dec = __C.ARCH['dec']
