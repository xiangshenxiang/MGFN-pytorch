...
from openvqa.ops.loss import My_loss
from openvqa.utils1.optim import get_optim, adjust_lr
...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    pretrained_emb = dataset.pretrained_emb #20573,300
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                # #标记
                # sub_frcn_feat_iter.requires_grad_(True)
                # print(sub_frcn_feat_iter)
                ...
                # print(sub_ans_iter.sum(-1),"mark1")
                # loss = loss_fn(loss_item[0], loss_item[1])
                # visual_grad = torch.autograd.grad((loss_item[0] * (loss_item[1] > 0).float()).sum(), sub_frcn_feat_iter,create_graph=True)[0]
                # # word_grad = torch.autograd.grad((pred * (a > 0).float()).sum(), word_emb, create_graph=True)[0]
                # visual_grad_cam = visual_grad.sum(2)[:, :44]
                # print(visual_grad_cam,"mark2")
                # visual_grad_cam = torch.softmax(visual_grad_cam[:, :44], dim=1)
                # print(visual_grad_cam,"mark3")
                # torch.save(visual_grad_cam, '495956002.pt')
                #
                # i = 1 / 0
                ##关键
